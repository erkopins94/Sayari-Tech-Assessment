...
1. **Resolution** — Entity names from the provided list were resolved to canonical Sayari entity IDs using the resolution endpoint. Rather than blindly accepting the single best fit, the resolver captures the **top-5 candidates** and a **confidence band** for each name, and flags low-confidence matches for human review (see *Name resolution ambiguity* below and the **Resolution Review** tab).
...
A subtler risk is the *silent* false positive: a weak top match that still looks "resolved." For example, *"State Development Bank VEB.RF"* (Russia) initially matched **BANK BELVEB OJSC** (Belarus) at a low score — a different entity in a different jurisdiction. Taking `data[0]` unconditionally would have let that error flow into every downstream analytic and AI answer with no trace. I addressed this in two layers. First, the resolver requests the top-5 candidates (`limit=5`, with a widened `candidate_pool_size`) and classifies the best match's score into a confidence band. The endpoint also exposes a `minimum_score_threshold` that would auto-reject sub-threshold matches, but in a sanctions/compliance context silently *dropping* a weak match hides the very risk an analyst needs to see — so the design keeps every match and instead flags the weak ones. Second, the **Resolution Review** tab presents those low-confidence matches (with automatic jurisdiction-mismatch warnings) for an analyst to confirm, re-map to the correct candidate, or mark unresolved — writing the decision back to the cache with a `reviewed` / `manual_override` audit trail. This turns a hidden data-quality risk into an explicit, auditable, human-in-the-loop step.

**Interpreting the match score.** Calibrating the confidence flag meant first understanding what the resolution `score` actually represents. It is not a normalized 0–100 percentage — it is an unbounded relevance score, and across the 49 resolved entities it ranged from roughly 40 to 200. That distribution was telling: the bulk of matches clustered above ~125, with a distinct weak tail at 40 (VEB.RF), 80 (Cubametales), 107 (Rustec), and 118 (Rostec). I set the review threshold at **120** to isolate exactly that tail — high enough to catch the genuinely uncertain matches, but not so high that it floods the queue with confident ones. The score also has to be read alongside other signals rather than in isolation: VEB.RF scored lowest (40) yet is the *correct* entity, while Rustec scored higher (107) but matched a marginal trading company — so the threshold deliberately flags for *human review* rather than auto-accepting or auto-rejecting. The cutoff lives in a single named constant (`REVIEW_SCORE_THRESHOLD`), so it can be retuned as more ground-truth feedback accumulates without touching any other logic.
